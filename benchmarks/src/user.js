...
export class User {
    ...
    constructor({ comm, block_pool, key_pair, accounts, }) {
        ...
        this.comm =
            comm ||
                new Comm(`${SHA256(this._key_pair[0].getPublic().encode("hex"), this._key_pair[1].getPublic().encode("hex"))}`);
    ...
    is_for_me(block) {
        try {
            ...
            var temp_key = genKeyPair(SHA256(genPublic(block.public_key).mul(a).encode("hex")) // block.public_key is of type Point
            );
