...
class Block_pool {
  ...
  add({ new_receive, new_send, addresses, network /* to find if network is strong or not */ }) {
    ...
    new_receive = new_receive.map((block) => Block.is_valid(block));
    new_send = new_send.map((block) => Block.is_valid(block));
    ...
    this.addresses = this.addresses.map(data => {
      ...
      let block = new_set.find(block => (block.sender_public === data.public) && (block.timestamp > data.timestamp));
