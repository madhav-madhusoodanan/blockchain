...
class Block_pool {
    ...
    clear() {}
    add({pool, addresses}) {
        // first, verify the hashing. Dump those that dont match hashing if money is present
        this.pool = pool.map(block => Block.is_valid(block));
        // then, if it is a receive block, check for a corresponding send block
        
        // if the send block exists but something doesnt match, dont update address and delete the receive block
